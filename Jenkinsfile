...
node {
  ...
  stage('Getting Vulnerability scan data') {
    sh "if ! gcloud beta container images list-tags  gcr.io/edcop-dev/$user_id-$tool_name --format='value(TAGS,VULNERABILITIES)' --filter='tags=15' | awk '{print $2}' | grep -q CRITICAL; then exit 1;fi"
  }
